#!/usr/bin/env python3
# Compute 2D area fractions (%) per time step from an Exodus .e file.
...
def name_index_map(char_var_name: str, ds: Dataset) -> Dict[str, int]:
    if char_var_name not in ds.variables:
        return {}
    # Add debugging to inspect the char array (remove after fixing)
    char_arr = ds.variables[char_var_name][:]
    print(f"Debug: {char_var_name} shape={char_arr.shape}, dtype={char_arr.dtype}, sample={char_arr[0] if char_arr.size > 0 else 'empty'}")
    names = decode_names(char_arr)
    print(f"Decoded names: {names}")  # Remove after verifying
...
def get_element_values_for_var(ds: Dataset,
    ...
    key = varname.lower()
    # Debug: Print available variable names
    print(f"Debug: Looking for variable '{varname}'")
    print(f"Available element variables: {list(name_to_idx_elem.keys())}")
    print(f"Available nodal variables: {list(name_to_idx_nod.keys())}")
    # Prefer element vars
    ...
    with Dataset(exo_file, "r") as ds:
        ...
        for pv in phase_vars:
            ...
            for it in range(len(times)):
                ...
                arr[it] = 100.0 * float(np.dot(h_e, areas) / A_total)  # percent
            out[pv] = arr
        return times, out
...
def main():
    ap = argparse.ArgumentParser(description="Compute 2D area fractions (%) from Exodus .e file.")
    ap.add_argument("--exo", required=True, type=Path, help="Path to Exodus .e file")
    ap.add_argument("--vars", nargs="+", required=True, help="Phase indicator variable names (element or nodal)")
