...
InputParameters
CrystalPlasticityStressUpdateBase_abs::validParams()
{
...
CrystalPlasticityStressUpdateBase_abs::CrystalPlasticityStressUpdateBase_abs(
...
void
CrystalPlasticityStressUpdateBase_abs::setMaterialVectorSize()
{
...
void
CrystalPlasticityStressUpdateBase_abs::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CrystalPlasticityStressUpdateBase_abs Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CrystalPlasticityStressUpdateBase_abs Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityStressUpdateBase_abs Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CrystalPlasticityStressUpdateBase_abs Error: The specified HCP slip direction "
...
void
CrystalPlasticityStressUpdateBase_abs::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityStressUpdateBase_abs::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CrystalPlasticityStressUpdateBase_abs::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityStressUpdateBase_abs::calculateSchmidTensor(
...
void
CrystalPlasticityStressUpdateBase_abs::calculateShearStress(
...
void
CrystalPlasticityStressUpdateBase_abs::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityStressUpdateBase_abs::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityStressUpdateBase_abs::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityStressUpdateBase_abs::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityStressUpdateBase_abs::isConstitutiveStateVariableConverged(
