...
InputParameters
CrystalPlasticityStressUpdateBase_new::validParams()
{
  ...
  params.addParam<MooseEnum>(
      ...
      MooseEnum("BCC FCC HCP TETRAGONAL", "FCC"),
      "Crystal lattice type or representative unit cell, i.e., BCC, FCC, HCP, TETRAGONAL, etc.");
...
CrystalPlasticityStressUpdateBase_new::CrystalPlasticityStressUpdateBase_new(
...
void
CrystalPlasticityStressUpdateBase_new::setMaterialVectorSize()
{
...
void
CrystalPlasticityStressUpdateBase_new::getSlipSystems()
{
  ...
  if (_reader.getData().size() != _number_slip_systems)
    paramError("number_slip_systems",
  ...
  if (_crystal_lattice_type == CrystalLatticeType::HCP)
    transformHexagonalMillerBravaisSlipSystems(_reader);
           ...
           _crystal_lattice_type == CrystalLatticeType::TETRAGONAL)
  {
    Real c_a_ratio = (_crystal_lattice_type == CrystalLatticeType::TETRAGONAL) ? 
                      _unit_cell_dimension[2] / _unit_cell_dimension[0] : 1.0;
    ...
    for (const auto i : make_range(_number_slip_systems))
    {
      ...
      for (const auto j : index_range(_reader.getData(i)))
      {
        ...
        else
        {
          ...
          // Adjust slip direction for tetragonal structure
          if (_crystal_lattice_type == CrystalLatticeType::TETRAGONAL && j - LIBMESH_DIM == 2)
            _slip_direction[i](j - LIBMESH_DIM) *= (1.0 / c_a_ratio);
        }
      }
    }
  }
  ...
  if (orthonormal_error)
    mooseError("CrystalPlasticityStressUpdateBase_new Error: The slip system file contains a slip "
               "direction and plane normal pair that are not orthonormal in the Cartesian "
               "coordinate system.");
}

void
CrystalPlasticityStressUpdateBase_new::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CrystalPlasticityStressUpdateBase_new Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CrystalPlasticityStressUpdateBase_new Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityStressUpdateBase_new Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CrystalPlasticityStressUpdateBase_new Error: The specified HCP slip direction "
...
void
CrystalPlasticityStressUpdateBase_new::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityStressUpdateBase_new::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void CrystalPlasticityStressUpdateBase_new::sortSlipSystemsForDO22()
{
  if (_crystal_lattice_type != CrystalLatticeType::TETRAGONAL)
    return; // Only apply for tetragonal structures

  std::vector<unsigned int> primary, secondary;

  for (unsigned int i = 0; i < _number_slip_systems; ++i)
  {
    // Compute the norm (magnitude) of the slip plane normal
    Real norm_value = _slip_plane_normal[i].norm();

    // Check if the slip plane normal aligns with (001) within tolerance
    if (std::abs(norm_value - 1.0) < libMesh::TOLERANCE)
      primary.push_back(i);
    else
      secondary.push_back(i);
  }

  // Reorder slip systems: primary systems first, secondary after
  _sorted_slip_systems.clear();
  _sorted_slip_systems.insert(_sorted_slip_systems.end(), primary.begin(), primary.end());
  _sorted_slip_systems.insert(_sorted_slip_systems.end(), secondary.begin(), secondary.end());
}


void
CrystalPlasticityStressUpdateBase_new::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityStressUpdateBase_new::calculateSchmidTensor(
...
void
CrystalPlasticityStressUpdateBase_new::calculateShearStress(
...
void
CrystalPlasticityStressUpdateBase_new::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityStressUpdateBase_new::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityStressUpdateBase_new::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityStressUpdateBase_new::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityStressUpdateBase_new::isConstitutiveStateVariableConverged(
