...
#include "CrystalPlasticityStressUpdateBase_abs1.h"
...
InputParameters
CrystalPlasticityStressUpdateBase_abs1::validParams()
{
  ...
  params.addParam<MooseEnum>(
      ...
      MooseEnum("BCC FCC HCP DO22", "FCC"),
      "Crystal lattice type or representative unit cell, i.e., BCC, FCC, HCP, DO22, etc.");
...
CrystalPlasticityStressUpdateBase_abs1::CrystalPlasticityStressUpdateBase_abs1(
...
CrystalPlasticityStressUpdateBase_abs1::initQpStatefulProperties()
{
  ...
  _backstress[_qp].assign(_number_slip_systems, 0.0);
}

void
CrystalPlasticityStressUpdateBase_abs1::setMaterialVectorSize()
{
...
void
CrystalPlasticityStressUpdateBase_abs1::getSlipSystems()
{
  ...
  if (orthonormal_error)
    mooseError("CrystalPlasticityStressUpdateBase_abs1 Error: The slip system file contains a slip "
               "direction and plane normal pair that are not orthonormal in the Cartesian "
               "coordinate system.");
}

void
CrystalPlasticityStressUpdateBase_abs1::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CrystalPlasticityStressUpdateBase_abs1 Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CrystalPlasticityStressUpdateBase_abs1 Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityStressUpdateBase_abs1 Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CrystalPlasticityStressUpdateBase_abs1 Error: The specified HCP slip direction "
...
void CrystalPlasticityStressUpdateBase_abs1::transformDO22SlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
...
void
CrystalPlasticityStressUpdateBase_abs1::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityStressUpdateBase_abs1::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CrystalPlasticityStressUpdateBase_abs1::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityStressUpdateBase_abs1::calculateSchmidTensor(
...
void
CrystalPlasticityStressUpdateBase_abs1::calculateShearStress(
...
void
CrystalPlasticityStressUpdateBase_abs1::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityStressUpdateBase_abs1::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityStressUpdateBase_abs1::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityStressUpdateBase_abs1::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityStressUpdateBase_abs1::isConstitutiveStateVariableConverged(
