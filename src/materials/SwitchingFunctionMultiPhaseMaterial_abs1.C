...
#include "SwitchingFunctionMultiPhaseMaterial_abs1.h"
#include "MooseException.h"

registerMooseObject("belson324App", SwitchingFunctionMultiPhaseMaterial_abs1);
registerMooseObject("belson324App", ADSwitchingFunctionMultiPhaseMaterial_abs1);

template <bool is_ad>
InputParameters
SwitchingFunctionMultiPhaseMaterial_abs1Templ<is_ad>::validParams()
{
...
template <bool is_ad>
SwitchingFunctionMultiPhaseMaterial_abs1Templ<is_ad>::SwitchingFunctionMultiPhaseMaterial_abs1Templ(
...
template <bool is_ad>
void
SwitchingFunctionMultiPhaseMaterial_abs1Templ<is_ad>::computeQpProperties()
{
  GenericReal<is_ad> sum_p = 0.0;
  GenericReal<is_ad> sum_all = 0.0;

  for (unsigned int i = 0; i < _num_eta_p; ++i)
    sum_p += (*_eta_p[i])[_qp] * (*_eta_p[i])[_qp] * (*_eta_p[i])[_qp] *
             (6.0 * (*_eta_p[i])[_qp] * (*_eta_p[i])[_qp] - 15.0 * (*_eta_p[i])[_qp] + 10.0);

  for (unsigned int i = 0; i < _num_eta; ++i)
    sum_all += (*_eta[i])[_qp] * (*_eta[i])[_qp] * (*_eta[i])[_qp] *
               (6.0 * (*_eta[i])[_qp] * (*_eta[i])[_qp] - 15.0 * (*_eta[i])[_qp] + 10.0);
  ...
  for (unsigned int i = 0; i < _num_eta; ++i)
  {
    // First derivatives
    if (_is_p[i])
      (*_prop_dh[i])[_qp] = (30.0 * (*_eta[i])[_qp] * (*_eta[i])[_qp] - 60.0 * (*_eta[i])[_qp]) *
                             sum_notp / (sum_all * sum_all);
    else
      (*_prop_dh[i])[_qp] = (-30.0 * (*_eta[i])[_qp] * (*_eta[i])[_qp] + 60.0 * (*_eta[i])[_qp]) *
                             sum_p / (sum_all * sum_all);
    ...
    for (unsigned int j = 0; j < _num_eta; ++j)
    {
      if (i == j)
      {
        if (_is_p[i])
          (*_prop_d2h[i][j])[_qp] =
              (60.0 * sum_all * sum_notp - 180.0 * (*_eta[i])[_qp] * (*_eta[i])[_qp] * sum_notp) /
              (sum_all * sum_all * sum_all);
        else
          (*_prop_d2h[i][j])[_qp] =
              (-60.0 * sum_p * sum_all + 180.0 * (*_eta[i])[_qp] * (*_eta[i])[_qp] * sum_p) /
              (sum_all * sum_all * sum_all);
      }
      else if (_is_p[i] && _is_p[j])
        (*_prop_d2h[i][j])[_qp] =
            -180.0 * (*_eta[i])[_qp] * (*_eta[j])[_qp] * sum_notp / (sum_all * sum_all * sum_all);
      else if (!_is_p[i] && !_is_p[j])
        (*_prop_d2h[i][j])[_qp] =
            180.0 * (*_eta[i])[_qp] * (*_eta[j])[_qp] * sum_p / (sum_all * sum_all * sum_all);
      else
        (*_prop_d2h[i][j])[_qp] = (120.0 * sum_all - 180.0 * sum_notp) * (*_eta[i])[_qp] *
...
template class SwitchingFunctionMultiPhaseMaterial_abs1Templ<true>;
template class SwitchingFunctionMultiPhaseMaterial_abs1Templ<false>;
