...
ComputeCrackedStressmod::validParams()
{
  ...
  params.addParam<MaterialPropertyName>(                                                       
      "D_name", "degradation", "Name of material property for energetic degradation function.");   /*  params.addParam<MaterialPropertyName>(
      "kappa_name",
      "kappa_op",
...
ComputeCrackedStressmod::ComputeCrackedStressmod(const InputParameters & parameters)
  : DerivativeMaterialInterface<Material>(parameters),
    ...
    _h(getMaterialProperty<Real>("D_name")),                                                        _dhdc(getMaterialPropertyDerivative<Real>("D_name", getVar("c", 0)->name())),                   _d2hdc2(getMaterialPropertyDerivative<Real>( 
...
void
ComputeCrackedStressmod::initQpStatefulProperties()
{
  _stress[_qp].zero();
  _hist[_qp] = 0.0;
}
...
void
ComputeCrackedStressmod::computeQpProperties()
{
  const Real c = _c[_qp];
  ...
  RankFourTensor Ppos = I4sym; //_uncracked_stress[_qp].positiveProjectionEigenDecomposition(eigval, eigvec);
  ...
  _stress[_qp] =  _h[_qp] * _uncracked_stress[_qp];
  _dstress_dc[_qp] = _uncracked_stress[_qp] * _dhdc[_qp];
  _Jacobian_mult[_qp] = _h[_qp] * _uncracked_Jacobian_mult[_qp];

  // Compute energy and its derivatives
  _F[_qp] = hist_variable * _h[_qp] - G0_neg; // + _gc_prop[_qp] * c * c / (2 * _l[_qp]);
  _dFdc[_qp] = hist_variable * _dhdc[_qp]; // + _gc_prop[_qp] * c / _l[_qp];
  _d2Fdc2[_qp] = hist_variable * _d2hdc2[_qp];// + _gc_prop[_qp] / _l[_qp];

  // 2nd derivative wrt c and strain = 0.0 if we used the previous step's history varible
  if (_use_current_hist)
    _d2Fdcdstrain[_qp] = stress0pos * _dhdc[_qp];
